...
factorIndexes = unique([G.var2factors{V}]);
factors = F(factorIndexes);
assignments = repmat(A, d, 1);
possible_values = [1:d]';
assignments(:,V) = repmat(possible_values, 1, length(V));

numFactors = length(factors);
for factor_ind=1:numFactors
    factor = factors(factor_ind);
    factor_assignments = assignments(:, factor.var);
    assignments_values = GetValueOfAssignment(factor, factor_assignments);
    LogBS = LogBS + log(assignments_values);
end;
