...
%alleles_p1 = genotypesToAlleles(genotypeVarParentOne,:);
%alleles_p2 = genotypesToAlleles(genotypeVarParentTwo,:);


for p2=1:numGenotypes
  alleles_p2 = genotypesToAlleles(p2,:);
  for p1=1:numGenotypes
    alleles_p1 = genotypesToAlleles(p1,:);
    for c=1:numGenotypes
      for n1=1:2
        for n2=1:2
          allele_p1 = alleles_p1(n1);
          allele_p2 = alleles_p2(n2);
          gen_com = allelesToGenotypes(allele_p1,allele_p2);
          if (gen_com == c)
            pos = AssignmentToIndex([c p1 p2], genotypeFactor.card);
            genotypeFactor.val(pos) += 0.25;
          endif
        endfor
      endfor
    endfor
