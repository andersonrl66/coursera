...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
assignmentmat=repmat(A,d,1);
usewhich=[];
for i=1:length(V)
    assignmentmat(:,V(i))=[1:d]';
    usewhich=[usewhich,G.var2factors{V(i)}];
end
usewhich=unique(usewhich);
for i=1:length(usewhich)
    [disordered,inda,indb]=intersect([1:length(A)],F(usewhich(i)).var);
    add=log(GetValueOfAssignment(F(usewhich(i)),assignmentmat(:,inda)));
    LogBS=LogBS+add;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
