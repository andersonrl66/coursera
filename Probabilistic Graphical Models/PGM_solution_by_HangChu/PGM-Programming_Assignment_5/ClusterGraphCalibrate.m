...
function [P MESSAGES delta193 delta1540 delta172] = ClusterGraphCalibrate(P,useSmartMP)
...
for m = 1:length(edgeFromIndx),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [MESSAGES(i,j).var,inda,indb]=intersect(P.clusterList(i).var,P.clusterList(j).var);
    MESSAGES(i,j).card=P.clusterList(i).card(inda);
...
while (1),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    thefactor=P.clusterList(i);
    for k=1:N
        if (P.edges(k,i)==1) && (k~=j)
            thefactor=FactorProduct(thefactor,MESSAGES(k,i));
        end
    end
    summedvars=setdiff(thefactor.var,MESSAGES(i,j).var);
    thefactor=FactorMarginalization(thefactor,summedvars);
    thefactor.val=thefactor.val/sum(thefactor.val);
    MESSAGES(i,j)=thefactor;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %code for quiz
    if i==19 && j==3
        num1=num1+1;
        delta193(num1)=MessageDelta(prevMessage,MESSAGES(19,3));
    end
    if i==15 && j==40
        num2=num2+1;
        delta1540(num2)=MessageDelta(prevMessage,MESSAGES(15,40));
    end
    if i==17 && j==2
        num3=num3+1;
        delta172(num3)=MessageDelta(prevMessage,MESSAGES(17,2));
    end
