...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % d_hid_hid = (hid_output.*(1 - hid_output));
  % fprintf('%d  %d \n',size(d_hid_hid,1),size(d_hid_hid,2));
  % term2 = d_hid_hid * data.inputs';
  % fprintf('%d  %d \n',size(term2,1),size(term2,2));
  % ret.input_to_hid = ( model.hid_to_class' * (class_prob - data.targets) )' * term2 / N + wd_coefficient*model.input_to_hid;
  ret.input_to_hid = 1/(N)*(model.hid_to_class'*(class_prob-data.targets)).*(hid_output-hid_output.^2)*data.inputs'+wd_coefficient*model.input_to_hid;
