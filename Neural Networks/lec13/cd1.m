...
% The returned value is the gradient approximation produced by CD-1. It's of the same shape as <rbm_w>.
  ...
  visible_data = sample_bernoulli(visible_data);  % Question 9: Treat real-valued image data as conditional probabilities for a visible state.
  ...
  hidden_sample = sample_bernoulli(hidden_probabilities); % question 7

  % Compute the CD 1 configuration goodness gradient using these actual visible and hidden states
  % now.
  % Implementing Question 7 means we don't actually need reconstructed hidden state, we can just
  % use the hidden state probabilities.
  %reconstruction_goodness = configuration_goodness_gradient(visible_sample, hidden_sample); % question 7
  reconstruction_goodness = configuration_goodness_gradient(visible_sample, hidden_probabilities); % question 8
